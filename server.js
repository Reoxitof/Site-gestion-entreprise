...
async function initDB() {
  try {
    await pool.query(`CREATE TABLE IF NOT EXISTS ec_users (
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS ec_presences (
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS ec_planning (
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS ec_annonces (
    ...
    const c = await pool.query('SELECT COUNT(*) FROM ec_users');
...
app.get('/api/employes', auth, async (req, res) => {
  try { res.json((await pool.query('SELECT id,username,nom,prenom,poste,role,actif,created_at FROM ec_users ORDER BY nom')).rows); }
  catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.put('/api/employes/:id', admin, async (req, res) => {
  try {
    ...
    await pool.query('UPDATE ec_users SET nom=$1,prenom=$2,poste=$3,role=$4,actif=$5 WHERE id=$6',
      [nom, prenom, poste, role, actif !== undefined ? actif : true, req.params.id]);
    res.json({ success: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/employes/:id', admin, async (req, res) => {
  try { await pool.query('UPDATE ec_users SET actif=false WHERE id=$1', [req.params.id]); res.json({ success: true }); }
  catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/presences/pointer', auth, async (req, res) => {
  try {
    ...
    const ex = await pool.query('SELECT * FROM ec_presences WHERE user_id=$1 AND date=$2', [uid, today]);
    if (!ex.rows.length) {
      await pool.query('INSERT INTO ec_presences (user_id,date,heure_arrivee,statut) VALUES ($1,$2,$3,$4)', [uid, today, now, 'present']);
      res.json({ action: 'arrivee', heure: now });
    } else if (!ex.rows[0].heure_depart) {
      await pool.query('UPDATE ec_presences SET heure_depart=$1 WHERE user_id=$2 AND date=$3', [now, uid, today]);
...
app.delete('/api/planning/:id', admin, async (req, res) => {
  try { await pool.query('DELETE FROM ec_planning WHERE id=$1', [req.params.id]); res.json({ success: true }); }
  catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/annonces', auth, async (req, res) => {
  try {
    res.json((await pool.query(`SELECT a.*,u.nom as auteur_nom FROM ec_annonces a LEFT JOIN ec_users u ON a.auteur_id=u.id ORDER BY a.created_at DESC LIMIT 20`)).rows);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/annonces', admin, async (req, res) => {
  try {
    ...
    await pool.query('INSERT INTO ec_annonces (titre,contenu,priorite,auteur_id) VALUES ($1,$2,$3,$4)',
      [titre, contenu, priorite || 'normale', req.session.user.id]);
    res.json({ success: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/annonces/:id', admin, async (req, res) => {
  try { await pool.query('DELETE FROM ec_annonces WHERE id=$1', [req.params.id]); res.json({ success: true }); }
  catch(e) { res.status(500).json({ error: e.message }); }
});
